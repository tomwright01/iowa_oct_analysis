...
#                       aes(x=0.6,xend=1.4,y=med,yend=med))
  ...
  p <- p+scale_x_continuous(name='Region',
                            breaks=c(1,2,3),
                            labels=c('Inner','Outer','Macular'))
  p <- p+scale_y_continuous(name=expression(paste("Thickness (",mu,"m)")))
  p <- p + default_theme
  print(p)
  filename = file.path('Output','Etdrs','Rings',paste0(layer,'_test-all.eps'))
  ggsave(filename,p,width=4.5,height=3.5,units=c("in"))
}


for(layer in levels(data$layer)){
  subdata <- data[data$layer==layer & data$test %in% c(2,3),]
  subdata_x <- x[x$layer==layer,]
  p <- ggplot(subdata)
  #p <- p+geom_boxplot(data=subset(subdata,group=='control'),outlier.shape=NA)
#   p <- p+geom_rect(data=subset(subdata_x,region=='Fovea'),
...
#                       aes(x=0.6,xend=1.4,y=med,yend=med))
  ...
  p <- p + default_theme
  print(p)
  filename = file.path('Output','Etdrs','Rings',paste0(layer,'_test-2.eps'))
  ggsave(filename,p,width=4.5,height=3.5,units=c("in"))
}
